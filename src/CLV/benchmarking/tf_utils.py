...
def inference(train_model, test_dataloader, batch_size, dataset_name, prep):
    ...
    prediction_model = prediction_model.graph()
    for feature, target in test_dataloader:

        start_time = time.time()
        prediction = prediction_model.predict(feature, batch_size)
        end_time = time.time()
